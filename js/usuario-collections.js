...
(() => {
  ...
  const samples = [
    ...
    ['streamVotesPanel', 'VOTACIONES', `<div class="vote-sim-shell"><div class="vote-sim-toolbar"><span class="ui-badge">MODO CHAT</span><span class="ui-badge">SUPERVIVIENTE</span><strong>00:00 · VOTACIÓN CERRADA</strong></div>${voteCardsMarkup(voteCards,{winner:'A'})}<div class="vote-sim-actions"><button type="button" disabled>REVELAR CARTA GANADORA</button><button type="button" disabled>IR A PERKS DE SUPERVIVIENTE</button></div><p class="vote-sim-help">Después de revelar la ganadora, el streamer puede tocar cualquier otra carta para mostrar qué tenía. Si la carta ganadora indica 3 perks, la ruleta especial deja 1 slot vacío y gira exactamente 3 perks reales.</p><div class="vote-sim-separator"></div>${voteCardsMarkup(specialCards,{special:true,winner:'B'})}</div>`]
  ];
