...
(() => {
  ...
  const samples = [
    ...
    ['streamGiveawaysPanel', 'SORTEOS', `<div class="participant-list">${[['Agus_demo','TWITCH'],['Participante_de_prueba_con_nombre_largo','KICK'],['Luna_demo','TWITCH']].map(([name,platform]) => `<div class="participant-row"><span class="participant-identity"><span>${name}</span><small class="ui-badge">ACEPTADO</small></span><span>${platform}</span></div>`).join('')}</div>`],
