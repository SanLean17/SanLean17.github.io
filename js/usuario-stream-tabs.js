...
(()=>{
  ...
  function installCanonicalStreamSelects(){
    ...
    function enhance(select){
      if(!(select instanceof HTMLSelectElement)||select.hidden||select.dataset.uiEnhanced==='true')return;select.dataset.uiEnhanced='true';select.classList.add('ui-native-select-source');
      const root=document.createElement('div'),trigger=document.createElement('button'),menu=document.createElement('div');root.className='custom-select ui-select-enhanced';trigger.type='button';trigger.setAttribute('aria-haspopup','listbox');trigger.setAttribute('aria-expanded','false');trigger.innerHTML='<span></span><b></b>';menu.className='ui-select-options';menu.hidden=true;root.append(trigger,menu);select.insertAdjacentElement('afterend',root);
      const render=()=>{const selected=select.options[select.selectedIndex];trigger.querySelector('span').textContent=selected?.textContent||'SELECCIONAR...';trigger.disabled=select.disabled;menu.innerHTML='';[...select.options].forEach(option=>{const button=document.createElement('button');button.type='button';button.textContent=option.textContent;button.disabled=option.disabled;button.addEventListener('click',()=>{select.value=option.value;select.dispatchEvent(new Event('change',{bubbles:true}));menu.hidden=true;trigger.setAttribute('aria-expanded','false');render()});menu.appendChild(button)})};
      trigger.addEventListener('click',()=>{if(trigger.disabled)return;const opening=menu.hidden;closeAll(root);menu.hidden=!opening;trigger.setAttribute('aria-expanded',opening?'true':'false')});select.addEventListener('change',render);render();
    }
